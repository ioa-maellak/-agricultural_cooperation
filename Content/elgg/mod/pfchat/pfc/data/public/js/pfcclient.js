...
pfcClient.prototype = {
  ...
  {
    this.el_words     = proto('pfc_words');
    this.el_handle    = proto('pfc_handle');
    this.el_container = proto('pfc_container');
//    this.el_online    = proto('pfc_online');
    this.el_errors    = proto('pfc_errors');
...
//    Event.observe(this.el_words,     'keypress',  this.callbackWords_OnKeypress.bindAsEventListener(this), false);
  ...
  askNick: function(nickname,error_text)
  {
    ...
    var pfcp = this.getPrompt();//new pfcPrompt(proto('pfc_container'));
  ...
  handleResponse: function(cmd, resp, param)
  {
    ...
    else if (cmd == "privmsg" || cmd == "privmsg2")
    {
      ...
      {
        this.displayMsg( cmd, this.res.getLabel('Maximum number of private pfchat has been reached') );
      }
    ...
    else if (cmd == "nick")
    {
      ...
      else if (resp == "notallowed")
      {
        ...
        // the 'notallowed' status. It will display a message and stop pfchat update.
        // If the pfchat update is not stopped, this will loop forever 
        // as long as the forced nickname is not changed.

        // display a message
        this.setError(this.res.getLabel('Chosen nickname is not allowed'), Array());
        // then stop pfchat updates
        this.updateChat(false);
        this.isconnected = false;
        this.refresh_loginlogout();
      }
  ...
  blink: function(id, action)
  {
    ...
    if (proto(id) == null) return;
    if (action == 'start')
    {
      this.blinktmp[id] = proto(id).style.backgroundColor;
      clearTimeout(this.blinktimeout[id]);
      this.blinktimeout[id] = setTimeout('pfc.blink(\'' + id + '\',\'loop\')', 500);
    }
    if (action == 'stop')
    {
      proto(id).style.backgroundColor = this.blinktmp[id];
    }
    if (action == 'loop')
    {
      if (this.blinkloop[id] == 1)
      {
        proto(id).style.backgroundColor = '#FFDFC0';
        this.blinkloop[id] = 2;
      }
      else
      {
        proto(id).style.backgroundColor = '#FFFFFF';
        this.blinkloop[id] = 1;
      }
      this.blinktimeout[id] = setTimeout('pfc.blink(\'' + id + '\',\'loop\')', 500);
    }
...
*/
  ...
  handleComingRequest: function( cmds )
  {
    ...
    for( var i=0; i<keys.length; i++)
    {
      ...
      var elt = proto('pfc_msg_'+recipientid+'_'+limit_msgid);
      while (elt)
      {
        ...
        if(elt.parentNode)
          ...
          elt.innerHTML = '';
        limit_msgid--;
        elt = proto('pfc_msg_'+recipientid+'_'+limit_msgid);
      }
    }
  ...
  sendRequest: function(cmd, recipientid)
  {
    ...
    if (cmd != "/update")
    {
      // setup a new timeout to update the pfchat in 5 seconds (in refresh_delay more exactly)
      clearTimeout(this.timeout);
      this.timeout = setTimeout('pfc.updateChat(true)', delay);
      this.timeout_time = new Date().getTime() + delay;
    ...
    new Ajax.Request(url, {
      ...
      onComplete: function(transport) {
        ...
        if (proto('pfc_ping')) proto('pfc_ping').innerHTML = this.ping+'ms'+' ['+parseInt(this.calcDelay() / 1000)+'s]';
      }.bind(this)
    });
  },
  ...
  updateNickWhoisBox: function(nickid)
  {
    ...
    if (pfc.getUserMeta(nickid,'nick') != this.nickname)
    {
      ...
      // Elgg-Profile Page Link
      var p = document.createElement('p');
      p.setAttribute(className, 'pfc_nickwhois_pv');
      var a = document.createElement('a');
      a.setAttribute('href', 'profile/'+pfc.getUserMeta(nickid,'nick'));
      ...
      img.alt = document.createTextNode('Profile');
      a.appendChild(img);
...
*/
  ...
  {
    var nickmarker_icon = proto('pfc_nickmarker');
    if (!root) root = proto('pfc_channels_content');
    if (this.nickmarker)
    {
  ...
  {
    var clock_icon = proto('pfc_clock');
    if (!root) root = proto('pfc_channels_content');
    if (this.clock)
    {
  ...
  {
    var loginlogout_icon = proto('pfc_loginlogout');
    if (this.isconnected)
    {
  ...
  /**
   * Minimize/Maximized the pfchat zone
   */
  ...
  {
    var content = proto('pfc_content_expandable');
    var btn     = proto('pfc_minmax');
    // ELGG: adjust he window height as well
    var win_height;
    if (this.minmax_status)
    {
    ...
    if (window.PFC_POPUP_WIN) {
      if (window.innerHeight) {
        window.innerHeight = win_height;
      } else if (document.body.clientHeight) {
        document.body.clientHeight = win_height;
      } else if (document.documentElement.clientHeight) {
        document.documentElement.clientHeight = win_height;
      }
    }
  },
  ...
  switch_text_color: function(color)
  {
    ...
    var colorbtn = this.getElementsByClassName(proto('pfc_colorlist'), 'pfc_color', '');
    ...
    proto(idname).style.border = '1px solid #555';
    proto(idname).style.padding = '1px';
  ...
  refresh_Smileys: function()
  {
    ...
    var btn = proto('pfc_showHideSmileysbtn');
    if (this.showsmileys)
    {
      if (btn)
      {
  ...
  refresh_WhosOnline: function()
  {
    ...
    var root = proto('pfc_channels_content');
    ...
    var btn = proto('pfc_showHideWhosOnlineBtn');
    if (!btn) return;
    if (this.showwhosonline)
    {
  ...
  {
    if (!this.pfc)
    this.pfc = new pfcPrompt(proto('pfc_container'));
    return this.pfc;
  }
};
